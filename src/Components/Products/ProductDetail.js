...
const ProductDetail = ( productID ) => {
  ...
  const getProductID = async () => {
    console.log("UseEffect productID res = ", productID);
    const result = await axios.get(
      //productID comes from prop "productID"
  ...
  useEffect(() => {
    ...
    // console.log("UseEffect productID res = ", productID);
    executeAxios("https://dummyjson.com/products/1")
  }, []);
  ...
  const buildProductsByIdDiv = (id) => {

    if(id.length === 0) return null;

    console.log("Build Product result = ", id)
...
{/* Description goes here */}
  ...
  {/* Wasn't sure if I should put 10.a.iii here */}
  ...
  return (<>{buildProductsByIdDiv(productByID)}</>);
};
